...
export let myUid=null, myName="ANON", myMoney=1000, myStats={games:0, wpm:0, wins:0}; 
export let myAchievements = []; 
export let myInventory = []; 
export let transactionLog = [];
export let globalVol=0.5, currentGame=null; 
export let keysPressed = {}; 
const audioCtx = new (window.AudioContext||window.webkitAudioContext)();

export let pAnim, sAnim, rAnim, gAnim, fAnim, typeInterval;
export let lossStreak = 0;
...
export function playSuccessSound() { beep(523.25, 'triangle', 0.1); setTimeout(() => beep(659.25, 'triangle', 0.1), 100); setTimeout(() => beep(783.99, 'triangle', 0.2), 200); }
export function setText(id, txt) { const el = document.getElementById(id); if(el) el.innerText = txt; }
export function logTransaction(msg, amount) { transactionLog.unshift({ msg, amount, ts: new Date().toLocaleTimeString() }); if(transactionLog.length > 20) transactionLog.pop(); updateBankLog(); }
export function updateBankLog() { const div = document.getElementById('bankLog'); div.innerHTML = transactionLog.map(t => `<div class="bank-entry"><span>${t.ts} ${t.msg}</span><span style="color:${t.amount>=0?'#0f0':'#f00'}">${t.amount>=0?'+':''}$${t.amount}</span></div>`).join(''); }
...
export function setMyMoney(val) { myMoney = val; }
export function setMyStats(val) { myStats = val; }
export function setMyName(val) { myName = val; }
export function setMyAchievements(val) { myAchievements = val; }
export function setMyInventory(val) { myInventory = val; }
export function setCurrentGame(val) { currentGame = val; }
export function setKeysPressed(key, val) { keysPressed[key] = val; }
export function setGlobalVol(val) { globalVol = val; }
export function setLossStreak(val) { lossStreak = val; }
export function setMyUid(val) { myUid = val; }

export function getMyMoney() { return myMoney; }
export function getMyStats() { return myStats; }
export function getMyName() { return myName; }
export function getMyAchievements() { return myAchievements; }
export function getMyInventory() { return myInventory; }
export function getCurrentGame() { return currentGame; }
export function getKeysPressed() { return keysPressed; }
...
export function getRank(money) { if(money < 500) return "RAT"; if(money < 2000) return "SCRIPT KIDDIE"; if(money < 5000) return "HACKER"; if(money < 10000) return "GOONER"; if(money < 50000) return "CYBER LORD"; return "KINGPIN"; }
function loadProfile(data) { myName = data.name; myMoney = data.money; myStats = data.stats || {games:0, wpm:0, wins:0}; myAchievements = data.achievements || []; myInventory = data.inventory || []; updateUI(); document.getElementById('overlayLogin').classList.remove('active'); localStorage.setItem('goonerUser', myName); localStorage.setItem('goonerPin', data.pin); if(myInventory.includes('item_matrix')) { document.documentElement.style.setProperty('--accent', '#00ff00'); document.getElementById('matrixCanvas').classList.add('active'); } if(myInventory.includes('item_rainbow')) document.body.classList.add('rainbow-mode'); if(myInventory.includes('item_flappy')) document.getElementById('btnFlappy').style.display = 'block'; const lastLogin = data.lastLogin || 0; const now = Date.now(); if(now - lastLogin > 86400000) { myMoney += 100; showToast("DAILY BONUS: $100", "💰"); } updateDoc(doc(db, 'gooner_users', myName), { lastLogin: now }); }
export function updateUI() { setText('displayUser', myName); const bankEl = document.getElementById('globalBank'); const bankOverlayEl = document.getElementById('bankDisplay'); const currentVal = parseInt(bankEl.innerText) || 0; if(currentVal !== myMoney) { bankEl.style.color = myMoney > currentVal ? '#0f0' : '#f00'; setTimeout(() => bankEl.style.color = 'var(--accent)', 500); } bankEl.innerText = myMoney; if(bankOverlayEl) bankOverlayEl.innerText = myMoney; setText('profName', myName); setText('profBank', "$"+myMoney); setText('profWPM', (myStats.wpm||0) + " WPM"); setText('profGames', myStats.games||0); setText('profUid', myUid ? myUid.substring(0,8) : "ERR"); const rank = getRank(myMoney); setText('displayRank', "[" + rank + "]"); setText('profRank', rank); if(myMoney >= 5000) unlockAchievement('diamond_hands'); if(myMoney >= 1000000) unlockAchievement('millionaire'); if(myMoney === 0) { unlockAchievement('rug_pulled'); myMoney = 10; logTransaction("EMERGENCY GRANT", 10); showToast("WELFARE GRANT", "💰", "Don't spend it all in one place."); saveStats(); } }
async function register(username, pin) { try { if(!myUid) return "WAITING FOR NETWORK..."; const ref = doc(db, 'gooner_users', username.toUpperCase()); const snap = await getDoc(ref); if(snap.exists()) return "USERNAME TAKEN"; const data = { name: username.toUpperCase(), pin: pin, money: 1000, joined: Date.now(), stats: {games:0, wpm:0, wins:0} }; await setDoc(ref, data); loadProfile(data); return true; } catch(e) { return "REG ERROR"; } }
export async function saveStats() { if(myName==="ANON") return; await updateDoc(doc(db, 'gooner_users', myName), { money: myMoney, stats: myStats, achievements: myAchievements, inventory: myInventory }); updateUI(); }

export function unlockAchievement(id) { if(myAchievements.includes(id) || myName === "ANON") return; myAchievements.push(id); const badge = ACHIEVEMENTS.find(a => a.id === id); if(badge) { myMoney += badge.reward; logTransaction(`ACHIEVEMENT: ${badge.title}`, badge.reward); showToast(`UNLOCKED: ${badge.title}`, badge.icon, `+$${badge.reward}`); } saveStats(); playSuccessSound(); }
export function renderBadges() { const grid = document.getElementById('badgeGrid'); grid.innerHTML = ''; ACHIEVEMENTS.forEach(a => { const unlocked = myAchievements.includes(a.id); const div = document.createElement('div'); if(a.hidden && !unlocked) { div.className = 'badge-item'; div.innerHTML = `<div class="badge-icon">🔒</div><div>???</div>`; div.onclick = () => showBadgeDetail(a, false); } else { div.className = 'badge-item unlocked ' + a.rarity; div.innerHTML = `<div class="badge-icon">${a.icon}</div><div>${a.title}</div>`; div.onclick = () => showBadgeDetail(a, true); } grid.appendChild(div); }); }
...
export function renderShop() { const list = document.getElementById('shopList'); setText('shopBank', myMoney); list.innerHTML = ''; SHOP_ITEMS.forEach(item => { const div = document.createElement('div'); div.className = 'shop-item'; let label = '$'+item.cost; let btnText = 'BUY'; let disabled = myMoney < item.cost; if(myInventory.includes(item.id) && item.type !== 'consumable') { label = 'OWNED'; btnText = 'ACTIVE'; disabled = true; } div.innerHTML = `<div>${item.name}<div style="font-size:8px;opacity:0.7">${item.desc}</div></div><div style="text-align:right"><span style="color:var(--accent)">${label}</span><button class="shop-buy-btn" onclick="window.buyItem('${item.id}')" ${disabled ? 'disabled' : ''}>${btnText}</button></div>`; list.appendChild(div); }); }
window.buyItem = (id) => { const item = SHOP_ITEMS.find(i => i.id === id); if(myMoney >= item.cost) { myMoney -= item.cost; if(item.type !== 'consumable') myInventory.push(id); else myInventory.push(id); if(id === 'item_rainbow') document.body.classList.add('rainbow-mode'); if(id === 'item_flappy') { document.getElementById('btnFlappy').style.display = 'block'; showToast("NEW GAME UNLOCKED", "🎮"); } if(myInventory.filter(i => i !== 'item_shield').length >= 3) unlockAchievement('shopaholic'); if(id === 'item_matrix') { unlockAchievement('neo'); document.documentElement.style.setProperty('--accent', '#00ff00'); document.getElementById('matrixCanvas').classList.add('active'); } logTransaction(`BOUGHT: ${item.name}`, -item.cost); saveStats(); renderShop(); playSuccessSound(); showToast(`BOUGHT: ${item.name}`, "🛒"); } };
export function showToast(title, icon, subtitle = "") { const t = document.createElement('div'); t.className = 'toast'; t.innerHTML = `<div class="toast-icon">${icon}</div><div class="toast-content"><div class="toast-title">${title}</div><div class="toast-desc">${subtitle}</div></div>`; document.getElementById('toastBox').appendChild(t); setTimeout(() => t.remove(), 4000); }
...
// SECRETS
const konamiCode = ['ArrowUp','ArrowUp','ArrowDown','ArrowDown','ArrowLeft','ArrowRight','ArrowLeft','ArrowRight','b','a']; let konamiIndex = 0; document.addEventListener('keydown', (e) => { if(e.key === konamiCode[konamiIndex]) { konamiIndex++; if(konamiIndex === konamiCode.length) { activateMatrixHack(); konamiIndex = 0; } } else konamiIndex = 0; });
function activateMatrixHack() { if(myName === "ANON") return alert("LOGIN FIRST"); document.documentElement.style.setProperty('--accent', '#00ff00'); document.getElementById('matrixCanvas').classList.add('active'); showToast("MATRIX MODE ACTIVATED", "🐇"); myMoney += 1000; saveStats(); playSuccessSound(); }
let logoClicks = 0; document.getElementById('mainBtn').onclick = () => { logoClicks++; if(logoClicks === 50) { unlockAchievement('spammer'); showToast("SECRET FOUND", "🤫", "500 Credits"); myMoney += 500; saveStats(); logoClicks = 0; } };
let bgClicks = 0; document.addEventListener('click', (e) => { if (e.target.tagName === 'BODY' || e.target.classList.contains('wrap')) { bgClicks++; if(bgClicks === 50) unlockAchievement('void_gazer'); } else { bgClicks = 0; } });
let chatCount = 0; function initChat() { const chatRef = collection(db, 'gooner_global_chat'); const q = query(chatRef, orderBy('ts', 'desc'), limit(15)); onSnapshot(q, (snap) => { const list = document.getElementById('chatHistory'); list.innerHTML = ''; const msgs = []; snap.forEach(d => msgs.push(d.data())); msgs.reverse().forEach(m => { const d = document.createElement('div'); d.className = 'chat-msg'; d.innerHTML = `<span class="chat-user">${m.user}:</span> ${m.msg}`; list.appendChild(d); }); list.scrollTop = list.scrollHeight; }); document.getElementById('chatInput').addEventListener('keypress', async (e) => { if(e.key === 'Enter') { const txt = e.target.value.trim(); if(txt.length > 0) { if(txt === '/clear') { document.getElementById('chatHistory').innerHTML=''; e.target.value=''; return;} if(txt === '/root') { e.target.value = ''; document.getElementById('hackOverlay').classList.add('active'); setTimeout(() => { document.getElementById('hackOverlay').classList.remove('active'); unlockAchievement('master_hacker'); }, 2000); return; } if(txt === '/help') { unlockAchievement('architect'); } chatCount++; if(chatCount === 10) unlockAchievement('chatterbox'); await addDoc(chatRef, { user: myName, msg: txt, ts: Date.now() }); e.target.value = ''; } } }); }

export function updateHighScore(game, score) { const k = `hs_${game}`; const old = parseInt(localStorage.getItem(k)||0); if(score > old) { localStorage.setItem(k, score); saveGlobalScore(game, score); return score; } return old; }
export function loadHighScores() { setText('hsPong', localStorage.getItem('hs_pong')||0); setText('hsSnake', localStorage.getItem('hs_snake')||0); setText('hsRunner', localStorage.getItem('hs_runner')||0); setText('hsGeo', localStorage.getItem('hs_geo')||0); setText('hsFlappy', localStorage.getItem('hs_flappy')||0); }
