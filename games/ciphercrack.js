...
function startRound(game, preserveChain = false) {
  if (!preserveChain) {
    game.chain.length = 0;
...
export function initCipherCrack() {
  ...
  canvas.onpointerdown = (event) => {
    ...
    if (game.chainSymbols.length === game.target.length) {
      ...
      for (let i = 0; i < game.board.length; i++) {
        if (Math.random() < 0.35) game.board[i].symbol = randomSymbol();
      }
      startRound(game);
      updateHighScore("ciphercrack", Math.floor(game.score));
      showToast("SEQUENCE DECRYPTED", "🔐");
      return;
    }
  ...
  const timer = window.setInterval(() => {
    ...
    game.shuffleInMs -= 100;

    game.entropy = Math.max(0, game.entropy - 0.3);

    if (game.shuffleInMs <= 0) {
      game.shuffleInMs = Math.max(3500, 9000 - game.round * 180);
      for (let i = 0; i < game.board.length; i++) {
        if (Math.random() < 0.25) game.board[i].symbol = randomSymbol();
      }
      game.chain.length = 0;
      game.chainSymbols.length = 0;
      startRound(game, true);
      showToast("MATRIX RESHUFFLED", "🧬");
    }
